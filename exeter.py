...
# ---------------------------------------------------------------------------
    ...
    print(f"""{Style.BRIGHT}
                                ...
                                # {Exeter.user.discriminator}{Fore.LIGHTYELLOW_EX}{Fore.LIGHTYELLOW_EX}     ||{Fore.LIGHTYELLOW_EX}  {Fore.RED}ID => {Fore.RED}{Exeter.user.id}
                                {Fore.RED}User =>  {Fore.RED}{Exeter.user.name}
...
Exeter = commands.Bot(description='Exeter ~ Free & OpenSource Selfbot',
...
async def on_message(message):
    ...
    if 'discordapp.com/gifts/' in message.content:
        if nitro_sniper == True:
            start = datetime.datetime.now()
            code = re.search("discordapp.com/gifts/(.*)",
                             message.content).group(1)
            token = config.get('token')
            headers = {'Authorization': token}
            r = requests.post(
...
async def on_guild_channel_create(channel):
    if Exeter.antiraid is True:
        try:
            guild = channel.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    guild.id].keys() and i.user.id is not Exeter.user.id:
                    print('not banned')
                else:
...
async def on_guild_channel_delete(channel):
    if Exeter.antiraid is True:
        try:
            guild = channel.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_delete):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    guild.id].keys() and i.user.id is not Exeter.user.id:
                    print('not banned')
                else:
...
async def on_guild_role_create(role):
    if Exeter.antiraid is True:
        try:
            guild = role.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.role_create):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    guild.id].keys() and i.user.id is not Exeter.user.id:
                    print('not banned')
                else:
...
async def on_guild_role_delete(role):
    if Exeter.antiraid is True:
        try:
            guild = role.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.role_delete):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    guild.id].keys() and i.user.id is not Exeter.user.id:
                    print('not banned')
                else:
...
@Exeter.command(aliases=["snipper"])
